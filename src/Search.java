...
public class Search {
	...
	public String AstarTreeSearch() {
		return printTree(new FrontierPriorityQueue(new ComparatorF(problem)));
	}
	...
	public String AstarGraphSearch() {
		return  AStarGraphSearch(new FrontierPriorityQueue(new ComparatorF(problem)));
	}

	// Iterative deepening, tree-search and graph-search
	/* depth = 20 */
	public String IterativeDeepeningTreeSearch() {
		return TreeSearchDepthLimited(new FrontierLIFO(), 20);
	}

	public String IterativeDeepeningGraphSearch() {
		return GraphSearchDepthLimited(new FrontierLIFO(), 20);
	...
	private String TreeSearchDepthLimited(Frontier frontier, int limit) {
		...
		while (true) {
			...
			if (!explored.contains(node.state) && i < limit) {
				explored.add(node.state);
				frontier.insertAll(Expand(node, problem));
				cnt++;
				i++;
	...
	private String Solution2(Node node) {
		double h, g, f;
		String s = "";
		g = node.path_cost;
		h = problem.h(node.state);
		f = g + h;
		String solution_str = node.state + " (g=" + g + ", h= " + h + ",f = " + f + ") " + "order = " + node.depth
				+ "\n";
	...
	private String printTree(Frontier frontier) {
		cnt = 0;
		node_list = new ArrayList<Node>();
	...
	private String AStarGraphSearch(Frontier frontier) {
		cnt = 0;
		node_list = new ArrayList<Node>();
		...
		while (true) {
			...
			if (!problem.goal_test(node.state))
				...
				path += Solution2(node);
				path = path +"\nTherefore the path is:"+ Solution(node);
				return path;
			}
